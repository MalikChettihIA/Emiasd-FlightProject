...
object FlightAvgDelayFeatureGenerator {
  ...
  def enrichFlightsWithAvgDelay(
    ...
    withUiLabels(
      groupId = "FlightAvgDelayFeatureGenerator.enrichFlightsWithAvgDelay",
      desc = "Flight Avg Delay Feature computation",
      tags = "data-pipeline,FlightAvgDelay,balanced,dataframe"
    ) {

      debug("")
      debug("=" * 80)
      debug("[AvgDelayFeatureGenerator] Flight Average Delay Generator - Start")
      debug("=" * 80)
      ...
      debug("[Step 1] Preparing timestamps...")
      ...
      debug("[Step 2] Creating window specification...")
      ...
      debug("[Step 3] Computing all statistics in single pass...")
  ...
  def enrichFlightsWithAvgDelayAggregated(
    ...
    info("- Calling com.flightdelay.data.preprocessing.flights.FlightAvgDelayFeatureGenerator.enrichFlightsWithAvgDelayAggregated")

    withUiLabels(
      groupId = "FlightAvgDelayFeatureGenerator.enrichFlightsWithAvgDelayAggregated",
      desc = "Flight Avg Delay Feature computation",
      tags = "data-pipeline,FlightAvgDelay,balanced,dataframe"
    ) {

      debug("[AvgDelayFeatureGenerator] Using AGGREGATED approach (for large datasets)")
  ...
  private def logMemoryUsage(label: String)(implicit spark: SparkSession, configuration: AppConfiguration): Unit = {
    ...
    debug(s"[$label] Memory:")
    debug(f"  Used: ${usedMemory}%,d MB / ${maxMemory}%,d MB ($usagePercent%%)")

    if (usagePercent > 80) {
      debug(s"    WARNING: Memory usage is ${usagePercent}% - Risk of OOM!")
      System.gc()
      Thread.sleep(1000)
    }
  }
