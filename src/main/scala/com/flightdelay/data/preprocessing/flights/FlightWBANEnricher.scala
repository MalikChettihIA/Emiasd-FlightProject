...
object FlightWBANEnricher {
  ...
  def preprocess(flightDf: DataFrame, originalWBANAirportTimezoneData: DataFrame)(implicit spark: SparkSession, configuration: AppConfiguration): DataFrame = {
    ...
    withUiLabels(
      groupId = "FlightWBANEnricher.preprocess",
      desc = "Enriching flights data with WBAN values",
      ...
      debug(s"  - Added UTC_CRS_DEP_TIME and UTC_FL_DATE conversion")
      debug(s"  - Handling timezone offsets and date boundaries")
      ...
      whenDebug{
        ...
        debug(s"Enrichment statistics:")
        debug(s"  - Total flights: $totalFlights")
        debug(s"  - Flights with origin WBAN: $flightsWithOriginWBAN (${(flightsWithOriginWBAN * 100.0 / totalFlights).round}%)")
        debug(s"  - Flights with destination WBAN: $flightsWithDestWBAN (${(flightsWithDestWBAN * 100.0 / totalFlights).round}%)")
      }

      debug("=" * 80)
      debug("[Preprocessing] Flight WBAN Enrichment - End")
      debug("=" * 80)
