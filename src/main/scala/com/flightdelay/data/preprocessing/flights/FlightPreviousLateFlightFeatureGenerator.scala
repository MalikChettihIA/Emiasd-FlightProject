...
object FlightPreviousLateFlightFeatureGenerator {
   ...
   */
  def createLateAircraftFeature(df: DataFrame)(implicit spark: SparkSession, configuration: AppConfiguration): DataFrame = {

    info("- Calling com.flightdelay.data.preprocessing.flights.FlightPreviousLateFlightFeatureGenerator.createLateAircraftFeature")

    withUiLabels(
      groupId = "FlightPreviousLateFlightFeatureGenerator.createLateAircraftFeature",
      desc = "Create Late Aircraft Feature",
      tags = "prep,semi-join,month-coverage"
    ) {
      debug("[PreviousLateFlightFeature] Creating PREV_AIRCRAFT_ARR_DELAY and turnaround_buffer features...")
      ...
      debug(s"  - Applying window function to compute previous flight features...")
      ...
      debug(s"  - Computing turnaround_buffer...")
      ...
      debug(s"   PREV_AIRCRAFT_ARR_DELAY and turnaround_buffer features created successfully")
